...
_shrinks_ the win from **−75%** to **−17%**. On Anthropic array-format traffic, `prune_stale`
...
- Claude Code sends **Anthropic array-format** tool results (`content` is an array of
  `tool_result` blocks). `pruneStaleResults()` has _always_ matched this shape, so
...
> does **NOT** mean the tests failed; it means the harness _scored nothing_ (it looks for a TAP
...
**This is conclusive — and the mirror image of DeepSeek.** Turning off _only_ `prune_stale`:
...
Honest note on the cache-hit "paradox": `optimize-nps` shows a _higher_ cache hit rate (99% vs
97%) yet a _higher_ cost. Not a contradiction — cache hit rate is a ratio, and cost tracks the
...
optimize run has the _best_ combined profile (cheapest + passes fixtures). On edge tests,
...
`compare.ts`, the full layer wins **−75% (batch 2) / −93% (batch 1)** _and_ delivers the best
...
Opposite conclusions for the _same_ strategy, with the `optimize-nps` control isolating
...
1. **`aap compare` showed `Fixture 0/0` / `Edge 0/0` although fixtures actually pass 54/54.**
   ...
   24's _default_ reporter emits the `ℹ`-prefixed summary (`ℹ pass 54`) instead — format
...
2. **The suite hung and was SIGKILLed** because the agent's `scheduler.js` can enter a
   **synchronous** busy-loop (`run()` skips its `await` when every queued task is permanently
   ...
   - _An in-process `setTimeout`/`Promise.race` guard can't interrupt a synchronous loop._
   ...
   - _`timeout -s KILL` on the parent `node --test` can orphan the per-file child processes_
...
flat-or-down, fixture tests 54/54, edge 54/57** — while the no-optimize baseline was the _only_
