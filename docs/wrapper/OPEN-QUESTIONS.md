...
## 1. Can the request stack be rewritten *outside* of `--resume`? (headline)
...
during a live session Claude holds the conversation in memory and only *appends* to
...
**Ask:** Is there *any* runtime mechanism to make a running Claude Code re-read /
...
## 2. Is there a hook that can *rewrite* (not just block) history or compaction?
...
**Ask:** After a rewind or `--resume`, can an event on an *abandoned* branch ever be
...
a real cache *hit* after a 6-minute idle gap on Bedrock in `eu-west-1` — only that the
request is accepted.
